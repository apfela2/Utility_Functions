tdep_PPV_biomarker <- function(predict.time = c(91.3, 182.6, 365.25, 547.8, 730.5), time_labels = c('91.3' = "3 Months", '182.6' = "6 Months", '365.25' = "12 Months", '547.8' = "18 Months", '730.5' = "24 Months"),
                               Stime, status, marker, data, plot.title, cut_length = 100, neg, xaxislab, xlimit = NULL) {
  ...
  ## Evaluate at chosen time points
  cutoff <-  seq(min(marker), max(marker), length.out = cut_length)
  SE <- purrr::map(cutoff, PPV_helper)
  
  
  PPV_SE <- unlist(lapply(lapply(SE, "[[", "inference"), "[[", "vect_se_PPV")) 
  NPV_SE <- unlist(lapply(lapply(SE, "[[", "inference"), "[[", "vect_se_NPV1")) 
  
  # PPV_data <- cbind(PPV_data, PPV_SE, NPV_SE)
  
  # Check
  if (PPV_SE[((cut_length*length(predict.time)/2) +2) ] != SE[[(cut_length/2 + 1)]]$inference$vect_se_PPV[2])
    stop("PPV_SE vector not aligned properly")
  
  
  if (PPV_data[((cut_length*length(predict.time)/2) +2) , "PPV"] != SE[[(cut_length/2 + 1)]]$PPV[2])
    stop("Vectors are not aligned properly")
  
  
  # PPV_data$PPV_ul <- PPV_data$PPV + 1.96*PPV_SE
  # PPV_data$PPV_ll <- PPV_data$PPV - 1.96*PPV_SE
  # PPV_data$NPV_ul <- PPV_data$NPV + 1.96*NPV_SE
  # PPV_data$NPV_ll <- PPV_data$NPV - 1.96*NPV_SE
  # 
  
  PPV_data <- tidyr::gather(PPV_data, key = "Metric", value = "Performance", PPV, NPV, Prevalence)
  
  PPV_data <- dplyr::mutate(.data = PPV_data, Metric = ifelse(Metric == "PPV", "NPV",
                                                              ifelse(Metric == "NPV", "PPV", Metric)))
  
  PPV_data <- dplyr::mutate(.data = PPV_data, Upper = ifelse(Metric == "PPV", Performance + 1.96*PPV_SE,
                                                             ifelse(Metric == "NPV", Performance + 1.96*NPV_SE, Performance)))
  
  PPV_data <- dplyr::mutate(.data = PPV_data, Lower = ifelse(Metric == "PPV", Performance - 1.96*PPV_SE,
                                                             ifelse(Metric == "NPV", Performance - 1.96*NPV_SE, Performance)))
  
  PPV_data <- dplyr::mutate(.data = PPV_data, Upper = ifelse(Upper > 1, 1, Upper))
  
  PPV_data <- dplyr::mutate(.data = PPV_data, Lower = ifelse(Lower < 0, 0, Lower))
  ...
  x.min.breaks <-  seq(round(min(PPV_data$cut, na.rm = T), 1),
                       round(max(PPV_data$cut, na.rm = T), 1), 0.2)
  ...
  if (is.null(xlimit)) {
    ...
    # Make plot
    p <-   ggplot(agg.dat, aes(y = Performance, x = cut, col = Metric, fill = Metric)) + 
      geom_ribbon(aes(ymin = Lower, ymax = Upper), alpha = .5, colour=NA) +
      geom_line(size = 1) +
