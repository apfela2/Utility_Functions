# Make time-dependent PPV-NPV plots for matrix of predictions

tdep_PPV_rep <- function(predict.time = c(91.3, 182.6, 365.25, 547.8, 730.5), time_labels = c('91.3' = "3 Months", '182.6' = "6 Months", '365.25' = "12 Months", '547.8' = "18 Months", '730.5' = "24 Months"),
                         Stime, status, marker, data, plot.title, cut_length = 100, limx = NULL) {
  ...
  for (i in 1:ncol(marker)) {
    ...
    PPV_data[[i]] <- tibble(cut = seq(min(marker), max(marker), length.out = cut_length)) %>%
      mutate(PPV = purrr::map(cut, PPV_helper),
             ...
             # Prevalence1 = as.vector(map_dfc(PPV, magrittr::extract2, "Stats")[2,4]),
  ...
  PPV_data_long <- bind_rows(PPV_data, .id = "rep")
  
  # Make prevalence a proportion
  PPV_data_long$Prevalence <- PPV_data_long$Prevalence/nrow(data)
  
  PPV_data_long <- tidyr::gather(PPV_data_long, key = "Metric", value = "Performance", PPV, NPV, Prevalence)
  
  PPV_data_long <- dplyr::mutate(.data = PPV_data_long, Metric = ifelse(Metric == "PPV", "NPV",
  ...
  x.min.breaks <-  seq(round(min(PPV_data_long$cut, na.rm = T), 1),
                       round(max(PPV_data_long$cut, na.rm = T), 1), 0.2)
  ...
  # Make plot
  p <- ggplot(agg.dat, aes(y = med, x = cut, col = Metric, fill = Metric)) + 
    geom_ribbon(aes(ymin = q025, ymax = q975), alpha = .5, colour=NA) +
    geom_line(size = 1) +
