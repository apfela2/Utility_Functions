# Calculate prediction matrix from ensemble output
calcRepCvPred_Earth <- function(ens, run.parallel = FALSE, refit = FALSE) {
  ...
  preds <- mclapply(seq_len(length(ens$models)), function(idx){ 
    ...
    # if (!refit) {
    this.pred <- predict(object = tm$selected.model, newdata = ens$dset[test.rows, ], type = "link")
  ...
  return(list(pred.mat = perf.mat, idxs = ens$outer.loop.folds))
}
