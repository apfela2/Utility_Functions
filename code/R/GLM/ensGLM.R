# Cross validation on GLM model
ensGLM <- function(dat, model_formula, run.parallel = FALSE, num.boot = 10,
                   outer.loop.cv.nfolds = NA, model_type = c("logistic", "cox"), response) {
  ...
  model.ens <- mclapply(seq_len(num.boot), function(nb) {
    ...
    if (model_type == "logistic") {
      models <- glm(formula = model_formula, family = "binomial", data = dat[this.row.idxs, , drop = FALSE])
    } else if (model_type == "cox") {
      models <- coxph(formula = model_formula, data = dat[this.row.idxs, , drop = FALSE])
    } else {
      stop("glm must be either logistic or cox")
    }
  ...
  return(list(models = model.ens, dset = dat, response = response, outer.loop.folds = rep.row.idxs))
}
