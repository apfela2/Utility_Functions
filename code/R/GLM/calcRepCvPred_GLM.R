# Calculate prediction matrix from ensemble output with main effects not included in interactions
calcRepCvPred_GLM <- function(ens, run.parallel = FALSE, refit = FALSE, main = F) {
  ...
  preds <- mclapply(seq_len(length(ens$models)), function(idx){ 
    ...
    # if (!refit) {
    this.pred <- predict(object = tm$selected.model, newdata = ens$dset[test.rows, ])
  ...
  return(list(pred.mat = perf.mat, idxs = ens$outer.loop.folds, formula = ens[["models"]][[1]][["selected.model"]][["formula"]][[3]][[3]]))
  } else {
    return(list(pred.mat = perf.mat, idxs = ens$outer.loop.folds, formula = ens[["models"]][[1]][["selected.model"]][["formula"]][[3]]))
  }
}
