ens_discGlinternet_foreach <- function(X, Y, numLevels, n.folds = 10, num.repeats = 30, fold.ids = NULL, lambda.type = "min", lsp = NULL, nlambda = 50, 
  ...
  if (is.logical(run.parallel) & run.parallel == TRUE) {
    require( doMC )
    registerDoMC( cores = max(1, (detectCores() - 1)) )
    ...
    require( doMC )
    registerDoMC( cores = run.parallel)
  }
  ...
  #-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#  
  model.ens <-foreach(nb = 1:num.boot, .packages = c("glinternet", "glmnet"), .verbose = TRUE) %dopar%
    {
