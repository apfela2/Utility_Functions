calcRepCvPred_discGlint <- function(ens, lambda_type = "min", run.parallel = TRUE, refit = FALSE) {
  ...
  full_temp <-discSurv::contToDisc(ens$X, ens$Y , ens$intervals)
  ...
  preds <- mclapply(seq_len(length(ens$models)), function(idx){ 
    ...
    test.rows  <- seq_len(nrow(ens$X))[-train.rows]
    
    
    this.s <- tm$selected.mod["lambda", lambda_type] #get value of lambda based on selected lambda type
    lambda_ind <- which(round(tm$enet.model[["lambda"]], digits = 9) == round(tm$selected.mod["lambda", lambda_type], digits = 9))
    
    # if (!refit) {
    
    
    cont_cov <- full_temp[test.rows,c(ens$cat_X, ens$col_cont_X, "timeDisc")]
    
    names(cont_cov)[names(cont_cov) == 'timeDisc'] <- 'timeInt'
    
    this.pred <- predict(object = tm$enet.model, X = cont_cov, lambda = tm$enet.model[["lambda"]][lambda_ind])
  ...
  perf.mat <- matrix(NA, nrow(ens$X), max(mod.map$cv.rep))
  ...
  return(list(pred.mat = perf.mat, coefs = coefs, idxs = ens$outer.loop.folds, dset = ens$X, time = ens$Y, intervals = ens$intervals))
}
