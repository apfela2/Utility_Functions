calcRepCvPred_glint <- function(ens, lambda_type = "min", run.parallel = TRUE, refit = FALSE) {
  ...
  preds <- mclapply(seq_len(length(ens$models)), function(idx){ 
    ...
    if (!refit) {
      this.pred <- predict(object = tm$enet.model, X = ens$dset[test.rows, ], lambda = tm$enet.model[["lambda"]][lambda_ind])
